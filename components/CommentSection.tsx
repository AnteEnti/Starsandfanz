...
interface CommentSectionProps {
  ...
  animatingFanzSayId?: string | null;
}

const MAX_DISPLAY_FANS = 5;

const CommentSection: React.FC<CommentSectionProps> = ({ fanzSays, onFanzSay, currentUserAvatar, animatingFanzSayId }) => {
  ...
  return (
    <>
        ...
        <div className="mt-5 pt-4 border-t border-slate-700">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {fanzSays.map(fs => {
              ...
              const isAnimating = animatingFanzSayId === fs.id;
              const animationClass = isAnimating ? 'animate-fanz-pop' : '';

              return (
                <button
                  key={fs.id}
                  onClick={() => onFanzSay(fs.id)}
                  className={`${buttonStyle} ${animationClass} text-sm py-1.5 px-3 rounded-full transition-colors duration-200`}
      ...
      <style>{`
        ...
        .animate-fanz-pop {
          animation: fanz-pop 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94);
        }
      `}</style>
    </>
  );
