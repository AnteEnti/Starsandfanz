...
const SuggestionCard: React.FC<SuggestionCardProps> = ({ suggestion, onToggleFan, onStartUnfan, disappearsOnFan = false, onViewCelebrityPage }) => {
  const { id, name, avatar, type, isFanned, linkedId } = suggestion;
  ...
  const canNavigate = type === SuggestionType.Celeb && linkedId;
  ...
  return (
    <div className={`relative flex-shrink-0 w-40 rounded-[10px] bg-gradient-to-r from-orange-400 via-fuchsia-500 to-indigo-500 p-[2px] animate-gradient-border transition-transform duration-300 transform hover:-translate-y-1 ${exitAnimationClass}`}>
      <div className="relative bg-slate-800 rounded-lg shadow-lg p-3 text-center h-full flex flex-col justify-between">
        {feedbackElement}
        <div 
          className={canNavigate ? 'cursor-pointer' : ''}
          onClick={() => canNavigate && onViewCelebrityPage(linkedId)}
          role={canNavigate ? 'button' : undefined}
          tabIndex={canNavigate ? 0 : -1}
          onKeyDown={(e) => {
            if (canNavigate && (e.key === 'Enter' || e.key === ' ')) {
              e.preventDefault();
              onViewCelebrityPage(linkedId);
            }
          }}
        >
          {cardContent}
